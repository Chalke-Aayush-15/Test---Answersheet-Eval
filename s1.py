...
# ========================================
# NEW: NLP SEMANTIC ANALYSIS IMPORTS
# ========================================
try:
...
# ========================================
# ENHANCED: ADVANCED ANSWER EVALUATOR WITH SEMANTIC ANALYSIS
# ========================================
class AdvancedAnswerEvaluator:
    """Advanced answer evaluation with semantic understanding, word overlap, and length bonus"""
    ...
    def __init__(self):
        ...
        # Evaluation weights
        self.weights = {
            'semantic': 0.60,      # 60% weight - semantic meaning
            'word_overlap': 0.30,   # 30% weight - keyword matching
            'length': 0.10         # 10% weight - completeness bonus
        }
        
        # Initialize the semantic model if available
    ...
    def extract_key_phrases(self, text):
        ...
        stop_words = {'the', 'a', 'an', 'is', 'are', 'was', 'were', 'be', 'been', 
                     'being', 'have', 'has', 'had', 'do', 'does', 'did', 'may', 
                     'might', 'must', 'can', 'could', 'will', 'would', 'shall', 
                     'should', 'of', 'in', 'on', 'at', 'by', 'for', 'with', 'about',
                     'to', 'from', 'as', 'and', 'or', 'but', 'if', 'then', 'else'}
        ...
        for i in range(len(key_words) - 1):
            phrases.append(' '.join(key_words[i:i+2]))
        for i in range(len(key_words) - 2):
            phrases.append(' '.join(key_words[i:i+3]))
        
        return set(key_words + phrases)
    ...
    def calculate_semantic_similarity(self, master_answer, student_answer):
        ...
        try:
            ...
            embeddings = self.model.encode([master_answer, student_answer], 
    ...
    def calculate_word_overlap_score(self, master_text, student_text):
        """Enhanced word overlap with key phrase matching"""
        # Preprocess texts
        master_processed = self.preprocess_text(master_text)
        student_processed = self.preprocess_text(student_text)
        
        if not master_processed or not student_processed:
            return 0.0
        
        # Extract words and key phrases
        master_words = set(master_processed.split())
        student_words = set(student_processed.split())
        
        master_phrases = self.extract_key_phrases(master_processed)
        student_phrases = self.extract_key_phrases(student_processed)
        
        # Calculate word overlap
        word_overlap = len(master_words.intersection(student_words))
        word_total = len(master_words)
        word_score = (word_overlap / word_total) * 8 if word_total > 0 else 0
        
        # Calculate phrase overlap (bonus)
        phrase_overlap = len(master_phrases.intersection(student_phrases))
        phrase_total = len(master_phrases)
        phrase_bonus = (phrase_overlap / phrase_total) * 2 if phrase_total > 0 else 0
        
        return min(10, word_score + phrase_bonus)
    
    def calculate_length_score(self, master_answer, student_answer):
        """Calculate length-based score with dynamic thresholding"""
        ...
        # Dynamic threshold based on master answer length
        expected_min = max(20, master_len * 0.4)  # At least 40% of master length
        expected_max = master_len * 2.0  # Up to 200% of master length
        
        if student_len < expected_min:
            # Linear scale for short answers
            return (student_len / expected_min) * 7  # Max 7 points for meeting minimum
        elif student_len <= expected_max:
            # Full points for good length
            return 10.0
        else:
            # Slight penalty for extremely long answers
            return 9.0
    
    def calculate_keyword_match_score(self, master_answer, student_answer):
        """Calculate keyword matching score for important terms"""
        # Extract important terms (capitalized words, numbers, technical terms)
        master_keywords = re.findall(r'\b[A-Z][a-z]*\b|\b\d+\b|\b\w{6,}\b', master_answer)
        student_keywords = re.findall(r'\b[A-Z][a-z]*\b|\b\d+\b|\b\w{6,}\b', student_answer)
        
        if not master_keywords:
            return 0.0
        
        # Convert to lowercase for matching
        master_set = set(k.lower() for k in master_keywords)
        student_set = set(k.lower() for k in student_keywords)
        
        matches = len(master_set.intersection(student_set))
        total = len(master_set)
        
        return (matches / total) * 10 if total > 0 else 0
    
    def evaluate_answer(self, master_answer, student_answer):
        """
        Enhanced evaluation with multiple components:
        1. Semantic Similarity (60%) - Understanding meaning
        2. Word Overlap + Phrases (30%) - Keyword matching
        3. Length Bonus (10%) - Completeness
        """
        if not student_answer or not student_answer.strip():
            return 0.0
        ...
        word_overlap_score = self.calculate_word_overlap_score(master_answer, student_answer)
        length_score = self.calculate_length_score(master_answer, student_answer)
        keyword_score = self.calculate_keyword_match_score(master_answer, student_answer)
        
        # Combine word overlap and keyword scores
        combined_overlap = (word_overlap_score * 0.7) + (keyword_score * 0.3)
        
        # Apply weights
        final_score = (
            semantic_score * self.weights['semantic'] +
            combined_overlap * self.weights['word_overlap'] +
            length_score * self.weights['length']
        )
        
        # Bonus for very high semantic similarity
        if semantic_score > 8.0:
            final_score = min(10, final_score * 1.1)
        
        # Ensure answer is at least 2 if it contains any relevant content
        if student_answer.strip() and len(student_answer) > 10:
            final_score = max(final_score, 2.0)
        
        return round(final_score, 2)
    
    def get_detailed_feedback(self, master_answer, student_answer, score):
        """Generate detailed feedback for the answer"""
        feedback = []
        
        # Length feedback
        if len(student_answer) < len(master_answer) * 0.3:
            feedback.append("Answer is too brief")
        elif len(student_answer) > len(master_answer) * 2:
            feedback.append("Answer is lengthy but covers concepts")
        ...
        if semantic_score > 7:
            ...
            feedback.append("Could improve conceptual clarity")
        ...
        if score >= 9:
            feedback.append("Outstanding answer!")
        elif score >= 7:
            feedback.append("Very good answer")
        elif score >= 5:
            feedback.append("Satisfactory answer")
        elif score >= 3:
            feedback.append("Needs improvement")
        else:
            feedback.append("Incomplete answer")
        
        return "; ".join(feedback[:3])  # Return top 3 feedback points
...
class MultiSubjectAnswerSheetEvaluator:
    """Main evaluation logic for multiple subjects with enhanced NLP evaluation"""
    ...
    def __init__(self, use_ocr=False):
        ...
        # ========================================
        # NEW: Initialize advanced evaluator
        # ========================================
        self.answer_evaluator = AdvancedAnswerEvaluator()
        self.log_messages.append(f"✅ NLP Semantic Analysis: {'Enabled' if self.answer_evaluator.semantic_enabled else 'Disabled'}")
        if not self.answer_evaluator.semantic_enabled:
            self.log_messages.append("   ℹ️ Using word overlap + length scoring only")
    ...
    def evaluate_answer(self, master_answer, student_answer):
        """
        NEW: Enhanced evaluation using semantic analysis + word overlap + length bonus
        """
        return self.answer_evaluator.evaluate_answer(master_answer, student_answer)
    ...
    def evaluate_subject(self, subject_data, log_widget=None):
        """Evaluate a single subject with enhanced scoring"""
        ...
        self.log(f"Evaluation Method: {'Semantic NLP + Word Overlap + Length' if self.answer_evaluator.semantic_enabled else 'Word Overlap + Length'}", log_widget)
        ...
        for i, student_pdf in enumerate(subject_data.student_pdfs):
            ...
            # Evaluate answers with enhanced scoring
            total_score = 0
            question_scores = {}
            ...
            for q_key, m_ans in master_answers.items():
                ...
                if score < 3 and s_ans:
                    self.log(f"  ⚠️ Q{q_key}: Low score ({score})", log_widget)
    ...
    def evaluate_all_subjects(self, log_widget=None, progress_callback=None):
        ...
        self.log(f"Evaluation Engine: {'Advanced NLP (Semantic + Overlap + Length)' if self.answer_evaluator.semantic_enabled else 'Standard (Word Overlap + Length)'}", log_widget)
        self.log("=" * 60, log_widget)
...
class MultiSubjectGUI:
    ...
    def __init__(self, root):
        ...
        # ========================================
        # NEW: Add semantic evaluation option
        # ========================================
    ...
    def create_subject_manager_tab(self):
        ...
        # Student PDFs - Individual files selection
        ttk.Label(right_panel, text="Student Answer Sheets:").grid(row=2, column=0, sticky='w', pady=5)
        
        # Frame for student PDFs controls
        student_pdfs_frame = ttk.Frame(right_panel)
        student_pdfs_frame.grid(row=2, column=1, padx=5, pady=5, sticky='ew')
        
        # Student PDFs listbox with scrollbar
        ttk.Label(student_pdfs_frame, text="Selected Student PDFs:").pack(anchor='w', pady=(0, 5))
        
        listbox_container = ttk.Frame(student_pdfs_frame)
        listbox_container.pack(fill='both', expand=True, pady=5)
        
        self.student_pdfs_listbox = tk.Listbox(listbox_container, height=6, font=("Arial", 9))
        ...
        ttk.Button(student_buttons_frame, text="➕ Add PDFs", 
                  command=self.add_student_pdfs).pack(side='left', padx=2)
        ttk.Button(student_buttons_frame, text="🗑️ Remove", 
                  command=self.remove_selected_student_pdf).pack(side='left', padx=2)
        ttk.Button(student_buttons_frame, text="📋 Clear All", 
                  command=self.clear_student_pdfs).pack(side='left', padx=2)
    ...
    def create_evaluation_tab(self):
        ...
        # ========================================
        # NEW: Add semantic evaluation checkbox
        # ========================================
        semantic_check_frame = ttk.Frame(options_frame)
        semantic_check_frame.pack(fill='x', pady=2)
        ttk.Checkbutton(semantic_check_frame, text="Use Semantic NLP Analysis (Recommended)", 
                       variable=self.use_semantic).pack(side='left', anchor='w')
        
        # Show semantic status
        if SEMANTIC_AVAILABLE:
            ttk.Label(semantic_check_frame, text="✅ Available", 
                     foreground="green", font=("Arial", 9, "bold")).pack(side='left', padx=5)
        else:
            ttk.Label(semantic_check_frame, text="❌ Not Installed", 
                     foreground="red", font=("Arial", 9, "bold")).pack(side='left', padx=5)
            ttk.Label(semantic_check_frame, text="(pip install sentence-transformers)", 
    ...
    def create_settings_tab(self):
        ...
        # NEW: NLP Settings
        # ========================================
        nlp_frame = ttk.LabelFrame(content_frame, text="NLP Settings", padding=15)
        nlp_frame.pack(fill='x', pady=(0, 15))
        
        ttk.Label(nlp_frame, text="Semantic Model:", font=("Arial", 10, "bold")).grid(row=0, column=0, sticky='w', pady=5)
        
        model_status = "✅ Installed" if SEMANTIC_AVAILABLE else "❌ Not Installed"
        model_color = "green" if SEMANTIC_AVAILABLE else "red"
        ttk.Label(nlp_frame, text=model_status, foreground=model_color).grid(row=0, column=1, sticky='w', pady=5)
        
        if not SEMANTIC_AVAILABLE:
            ttk.Label(nlp_frame, text="Install with: pip install sentence-transformers", 
                     foreground="blue").grid(row=1, column=0, columnspan=2, pady=5)
...
"""
    ...
    def start_evaluation(self):
        ...
        # ========================================
        # NEW: Update evaluator settings
        # ========================================
        self.evaluator.use_ocr = self.use_ocr.get()
        self.evaluator.answer_evaluator.semantic_enabled = self.use_semantic.get() and SEMANTIC_AVAILABLE
        ...
        # ========================================
        # NEW: Show evaluation method
        # ========================================
        eval_method = "Semantic NLP + Word Overlap + Length" if self.evaluator.answer_evaluator.semantic_enabled else "Word Overlap + Length (Basic)"
        self.log_message(f"Evaluation Method: {eval_method}")
        self.log_message("=" * 60)
